...
class APITestCase(TestCase):
    ...
    def test_api_can_get_list_of_boards(self):
        response = self.client.get('/board/v1/boards/')
        self.assertEqual(response.status_code, status.HTTP_200_OK)

    def test_api_can_get_detail_of_board(self):
        response = self.client.get('/board/v1/boards/{}/'.format(self.board.pk))
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_api_can_post_valid_move(self):
        response = self.client.post(
            '/board/v1/boards/{}/moves/'.format(self.board.pk),
            {
                "to_position": "e2e4"
            }
        )
    ...
    def test_api_get_recognizes_move(self):
        ...
        get_response = self.client.get('/board/v1/boards/{}/'.format(self.board.pk))
        self.assertEqual(
    ...
    def test_api_get_latest_board(self):
        ...
        get_response = self.client.get('/board/v1/boards/?top=1'.format(self.board.pk))
